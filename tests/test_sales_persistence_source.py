...
class SalesPersistenceSourceTests(unittest.TestCase):
    ...
    def test_invoice_object_is_freed_after_transaction_scope(self):
        routine = extract_routine(read_source(), "GravarNotaFiscal")
        commit = routine.index("FClient.Orm.Commit;")
        free = routine.index("NotaFiscal.Free;")

        self.assertLess(commit, free)
        self.assertIn("finally", routine)
    ...
    def test_payment_movements_are_freed_after_insert(self):
        routine = extract_routine(read_source(), "ProcessarParcelasNotaFiscal")

        create = routine.index("Financeiro := TOrmMoviFinanceiro.Create;")
        add = routine.index("FClient.Orm.Add(Financeiro, True);")
        free = routine.index("Financeiro.Free;")

        self.assertLess(create, add)
        self.assertLess(add, free)
        self.assertIn("finally", routine)
